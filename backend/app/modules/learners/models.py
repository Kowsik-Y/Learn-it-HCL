"""
Learner Module — Models and Router
...
class LearnerProfile(Base, TimestampMixin, TenantMixin):
    ...
    professional_experience: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
...
class LearnerPreferences(Base, TimestampMixin, TenantMixin):
    ...
    id: Mapped[uuid.UUID] = mapped_column(PG_UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    learner_id: Mapped[uuid.UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("learner_profiles.id"), nullable=False, unique=True)
    ...
    available_days: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
...
class LearnerGoal(Base, TimestampMixin, TenantMixin):
    ...
    id: Mapped[uuid.UUID] = mapped_column(PG_UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    learner_id: Mapped[uuid.UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("learner_profiles.id"), nullable=False)
    title: Mapped[str] = mapped_column(String(500), nullable=False)
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    goal_type: Mapped[str] = mapped_column(String(50), nullable=False, default="career")
    target_role: Mapped[str | None] = mapped_column(String(255), nullable=True)
    target_role_id: Mapped[uuid.UUID | None] = mapped_column(PG_UUID(as_uuid=True), nullable=True)
    ...
    known_skills: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    unknown_skills: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
