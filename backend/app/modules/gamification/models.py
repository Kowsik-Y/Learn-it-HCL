...
from sqlalchemy import String, Float, ForeignKey, Text, Integer, Boolean, Index, DateTime, Date, JSON
...
class Streak(Base, TimestampMixin, TenantMixin):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    learner_id: Mapped[str] = mapped_column(String(36), nullable=False, unique=True)
...
class Quest(Base, TimestampMixin, TenantMixin):
    ...
    tasks: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    xp_reward: Mapped[int] = mapped_column(Integer, default=50, nullable=False)
    badge_reward_id: Mapped[str | None] = mapped_column(String(36), nullable=True)
