...
import { 
  Loader2, CheckCircle2, XCircle, ShieldAlert, ShieldCheck, Mail, Building, Clock
} from "lucide-react";
...
import { MoreHorizontal, Trash2, Pencil, Eye, User } from "lucide-react";
...
export default function AdminUsersPage() {
  ...
  const handleProcessRequest = async (id: string, action: "approve" | "reject") => {
    ...
    try {
      const data = await api.request<{ user?: any }>(`/admin/requests/${id}/${action}`, { method: "POST" });
  ...
  const userColumns: Column<any>[] = [
    {
      ...
      render: (user) => (
        <>
          ...
          <div className={`text-xs text-muted-foreground ${!user.isActive ? "line-through" : ""}`}>{user.email}</div>
        </>
      ),
    },
    {
      ...
      render: (user) => user.organization?.name || <span className="text-muted-foreground italic">None</span>,
    },
    {
      header: "Role",
      cellClassName: (user: any) => !user.isActive ? "opacity-70" : "",
  ...
  return (
    <>
      ...
      {approvalResult && (
            <Dialog open={!!approvalResult} onOpenChange={() => setApprovalResult(null)}>
              <DialogContent>
                ...
                <p className="text-sm text-muted-foreground">Please securely share these temporary credentials with the user:</p>
                <div className="bg-background rounded p-4 font-mono text-sm border shadow-inner">
                  ...
                  <div><strong>Password:</strong> {approvalResult.password}</div>
                </div>
                <DialogFooter>
      ...
      <ConfirmDialog
        ...
        title="Delete User"
        description={
          <>Are you sure you want to permanently delete <span className="font-bold">{userToDelete?.name}</span>? This action cannot be undone and will remove all their data from the system.</>
        }
          ...
          <>
            <div className="space-y-1 border-b pb-4">
              ...
              <div className="flex items-center space-x-2">
                <Badge variant={viewingUser.isActive ? "default" : "secondary"} className={viewingUser.isActive ? "bg-green-500/10 text-green-600 border-green-500/20" : "bg-destructive/10 text-destructive border-destructive/20"}>
            ...
            <div className="space-y-4 border-b pb-4">
              ...
              {viewingUser.organization ? (
                <div className="space-y-2">
                  ...
                  <p className="font-mono text-xs text-muted-foreground">/{viewingUser.organization.slug}</p>
                </div>
              ) : (
      ...
      <Tabs defaultValue="requests" className="w-full">
        <TabsList className="mb-6 h-12 w-full md:w-auto bg-muted/50 p-1">
          <TabsTrigger value="requests" className="flex-1 md:flex-none text-sm font-medium h-10 px-6 rounded-md data-[state=active]:bg-background data-[state=active]:shadow-sm">
            Pending Requests <Badge variant="secondary" className="ml-2 bg-primary/10 text-primary">{pendingRequests.length}</Badge>
          </TabsTrigger>
          <TabsTrigger value="active" className="flex-1 md:flex-none text-sm font-medium h-10 px-6 rounded-md data-[state=active]:bg-background data-[state=active]:shadow-sm">
            Active Users <Badge variant="secondary" className="ml-2">{activeUsers.length}</Badge>
          </TabsTrigger>
          <TabsTrigger value="blocked" className="flex-1 md:flex-none text-sm font-medium h-10 px-6 rounded-md data-[state=active]:bg-background data-[state=active]:shadow-sm">
            Blocked Users <Badge variant="secondary" className="ml-2">{blockedUsers.length}</Badge>
          </TabsTrigger>
        </TabsList>
              ...
              {pendingRequests.map((req) => (
                <Card key={req.id} className="overflow-hidden hover:shadow-md transition-shadow">
                  ...
                  <CardContent className="pt-4 space-y-4">
                    ...
                    <div className="text-sm bg-muted/30 p-3 rounded-md italic">
                      "{req.reason}"
                    </div>
                    ...
                    <div className="flex gap-2 pt-2">
                      ...
                      >
                        {processingId === req.id ? <Loader2 className="h-4 w-4 animate-spin" /> : "Approve & Provision"}
                      </Button>
                      <Button 
                        size="sm" 
                        variant="outline" 
