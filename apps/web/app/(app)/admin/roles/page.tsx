...
export default function AdminRoles() {
  ...
  const fetchRoles = async () => {
    try {
      const data = await api.request<{ items: any[] }>("/admin/roles");
      setRoles(data.items);
    } catch (err) {
      console.error(err);
  ...
  const handleSaveRole = async (e: React.FormEvent) => {
    ...
    try {
      if (editingRoleId) {
        await api.request(`/admin/roles/${editingRoleId}`, {
          method: "PUT",
  ...
  const handleDeleteRole = async () => {
    ...
    try {
      await api.request(`/admin/roles/${roleToDelete.id}`, {
        method: "DELETE",
      });
      fetchRoles();
  ...
  if (loading) {
    return <div className="flex justify-center items-center h-64 text-muted-foreground">Loading global roles...</div>;
  }
  ...
  const columns: Column<any>[] = [
    ...
    {
      header: "Scope",
      render: () => <Badge variant="default">Global</Badge>,
    },
    {
      header: "Permissions",
      render: (role) =>
      ...
      render: (role) => (
        <DataTableRowActions
          ...
          actions={[
            ...
            !["super_admin", "org_admin"].includes(role.slug) && {
              label: "Delete Role",
  ...
  return (
    <>
      <PageHeader
        title="Global Roles"
        description="Manage platform-wide roles available to all tenants."
        actions={
      ...
      />
          <FormDialog
            ...
            title={editingRoleId ? "Edit Global Role" : "Create Global Role"}
            description={
              editingRoleId
                ? "Modify the name and permissions for this role."
                : "Create a new role that will be available to every organization."
            }
