    ...
    try {
      ...
      const needsReview = allSkills
        ...
        .map((s: any) => ({
          ...
          urgency:
            s.retention_estimate < 0.4
              ? ('critical' as const)
              : s.retention_estimate < 0.6
                ? ('due' as const)
                : ('upcoming' as const),
        }))
      ...
      setReviewCards([
        {
          ...
          mastery_score: 0.42,
          retention_estimate: 0.25,
          last_assessed_at: new Date(Date.now() - 7 * 86400000).toISOString(),
          evidence_count: 5,
          urgency: 'critical',
        },
        {
          ...
          last_assessed_at: new Date(Date.now() - 5 * 86400000).toISOString(),
          evidence_count: 4,
          urgency: 'critical',
        },
        {
          ...
          mastery_score: 0.55,
          retention_estimate: 0.45,
          last_assessed_at: new Date(Date.now() - 4 * 86400000).toISOString(),
          evidence_count: 6,
          urgency: 'due',
        },
        {
          ...
          mastery_score: 0.78,
          retention_estimate: 0.55,
          last_assessed_at: new Date(Date.now() - 3 * 86400000).toISOString(),
          evidence_count: 10,
          urgency: 'due',
        },
        {
          ...
          mastery_score: 0.48,
          retention_estimate: 0.6,
          last_assessed_at: new Date(Date.now() - 2 * 86400000).toISOString(),
          evidence_count: 6,
          urgency: 'upcoming',
        },
        {
          ...
          last_assessed_at: new Date(Date.now() - 1 * 86400000).toISOString(),
          evidence_count: 8,
          urgency: 'upcoming',
        },
      ]);
    ...
    return (
      <div className="container max-w-3xl py-8 space-y-8">
        <PageHeader
          ...
          description="FSRS-powered daily review queue."
        />
      ...
      <div className="container max-w-2xl py-16 space-y-8">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">
            You&apos;ve completed all {totalCount} reviews for today. Your memory retention has been
            reinforced.
          </p>
        </div>
        <Card>
    ...
    return (
      <div className="container max-w-2xl py-16 space-y-8">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">
            All your skills have strong retention. Come back tomorrow or take a quiz to add skills
        ...
        {['critical', 'due', 'upcoming'].map((urgency) => {
          ...
          return (
            <Badge
              key={urgency}
              variant="outline"
              className={`${config.bg} ${config.color} border-0 gap-1 px-3 py-1`}
            >
      ...
      <div className="space-y-4">
        {pendingCards.map((card) => {
          ...
          return (
            <Card
              key={card.skill_id}
              className={`transition-all ${config.border} ${isActive ? 'ring-2 ring-primary/30' : ''}`}
            >
                  ...
                  <div>
                    ...
                    <CardDescription className="flex items-center gap-3 mt-1">
                      <span className="flex items-center gap-1">
                        ...
                        {(card.mastery_score * 100).toFixed(0)}%
                      </span>
                      <span className="flex items-center gap-1">
              ...
              <CardContent className="space-y-3">
                ...
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    ...
                    <span
                      className={`font-bold ${card.retention_estimate < 0.4 ? 'text-red-500' : card.retention_estimate < 0.6 ? 'text-amber-500' : ''}`}
                    >
                ...
                <p className="text-xs text-muted-foreground bg-muted/50 rounded-lg p-2.5">
                  📊 FSRS predicts your memory for this skill has decayed to{' '}
                  <strong>{(card.retention_estimate * 100).toFixed(0)}%</strong>.
                  {card.retention_estimate < 0.5
                    ? ' Review now to prevent further forgetting!'
                    : ' A quick review will reinforce retention.'}
                </p>
              </CardContent>
