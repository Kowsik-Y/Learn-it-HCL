...
import { Brain, CheckCircle2, Send, Sparkles, User } from 'lucide-react';
...
export default function OnboardingPage() {
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'assistant',
      content:
        "Hi! I'm your Learn-it AI Guide. I'm here to help set up your personalized learning path. What are your main career or learning goals?",
    },
  ]);
            ...
            <div className="w-full bg-card border border-border rounded-lg p-4 mb-8 text-left space-y-3">
              <div>
                ...
                <div className="font-medium text-sm">
                  {extractedData?.goal || 'Become a Full-Stack Python Developer'}
                </div>
              </div>
              <div>
                <span className="text-xs text-muted-foreground uppercase font-semibold tracking-wider">
                ...
                <div className="font-medium text-sm">
                  {extractedData?.known_skills?.length
                    ? extractedData.known_skills.join(', ')
              ...
              <div>
                ...
                <div className="font-medium text-sm">
                  {extractedData?.target_role || 'Backend Software Engineer'}
                </div>
              </div>
            </div>
  ...
  return (
    <div className="flex-1 flex flex-col bg-background max-w-3xl mx-auto my-4 border border-border rounded-xl overflow-hidden shadow-xs">
      {/* Header */}
      <header className="px-6 py-4 border-b border-border bg-card/50 backdrop-blur flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-foreground">AI Guide</h1>
      ...
      <ScrollArea className="flex-1 p-6 h-[400px]" ref={scrollRef}>
        <div className="flex flex-col gap-6 pb-4">
          {messages.map((msg, i) => (
            ...
            >
              <Avatar
                className={`h-8 w-8 ${msg.role === 'assistant' ? 'bg-primary/20 text-primary' : 'bg-muted text-muted-foreground'}`}
              >
                <AvatarFallback>
                  {msg.role === 'assistant' ? (
      ...
      <div className="p-4 bg-background border-t border-border">
        <form onSubmit={handleSend} className="flex gap-3 max-w-3xl mx-auto relative items-end">
          <Input
            value={input}
            onChange={(e) => setInput(e.target.value)}
            placeholder="Type your answer..."
            disabled={loading}
            className="flex-1 min-h-12 rounded-xl pr-14"
          />
          <Button
            type="submit"
            disabled={!input.trim() || loading}
            size="icon"
            className="absolute right-2 bottom-1 h-10 w-10 rounded-lg font-bold"
          >
