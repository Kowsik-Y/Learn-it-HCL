  ...
  const loadAnalytics = async () => {
    ...
    const demoStudents: StudentRisk[] = [
      {
        id: 's1',
        name: 'Alex Thompson',
        email: 'alex@learnit.dev',
        risk_score: 85,
        risk_level: 'critical',
        suggested_nudge: "Send a personal check-in message. Student hasn't logged in for 12 days.",
        days_inactive: 12,
        current_streak: 0,
        mastery_avg: 0.25,
      },
      {
        id: 's2',
        name: 'Maya Patel',
        email: 'maya@learnit.dev',
        risk_score: 72,
        risk_level: 'high',
        suggested_nudge: 'Recommend easier practice material — 4 consecutive failures detected.',
        days_inactive: 5,
        current_streak: 0,
        mastery_avg: 0.35,
      },
      {
        id: 's3',
        name: 'Jordan Lee',
        email: 'jordan@learnit.dev',
        risk_score: 55,
        risk_level: 'moderate',
        suggested_nudge: 'Encourage spaced repetition review — retention dropping.',
        days_inactive: 3,
        current_streak: 2,
        mastery_avg: 0.52,
      },
      {
        id: 's4',
        name: 'Sam Rivera',
        email: 'sam@learnit.dev',
        risk_score: 40,
        risk_level: 'moderate',
        suggested_nudge: 'Suggest taking the adaptive diagnostic quiz to identify gaps.',
        days_inactive: 2,
        current_streak: 4,
        mastery_avg: 0.6,
      },
      {
        id: 's5',
        name: 'Priya Sharma',
        email: 'priya@learnit.dev',
        risk_score: 15,
        risk_level: 'low',
        suggested_nudge: 'On track! Consider recommending advanced challenges.',
        days_inactive: 0,
        current_streak: 14,
        mastery_avg: 0.82,
      },
      {
        id: 's6',
        name: 'Chris Kim',
        email: 'chris@learnit.dev',
        risk_score: 10,
        risk_level: 'low',
        suggested_nudge: 'Excellent progress. Ready for project-based assessment.',
        days_inactive: 0,
        current_streak: 21,
        mastery_avg: 0.91,
      },
      {
        id: 's7',
        name: 'Taylor Wu',
        email: 'taylor@learnit.dev',
        risk_score: 65,
        risk_level: 'high',
        suggested_nudge: 'Student struggling with SQL topics. Consider 1-on-1 support.',
        days_inactive: 4,
        current_streak: 0,
        mastery_avg: 0.38,
      },
      {
        id: 's8',
        name: 'Robin Clarke',
        email: 'robin@learnit.dev',
        risk_score: 28,
        risk_level: 'low',
        suggested_nudge: 'Consistent learner. Encourage peer mentoring.',
        days_inactive: 1,
        current_streak: 8,
        mastery_avg: 0.72,
      },
    ...
    const demoHeatmap: SkillHeatmapRow[] = demoStudents.map((s) => ({
      ...
      skills: Object.fromEntries(
        skills.map((sk) => [
          sk,
          Math.max(0, Math.min(1, s.mastery_avg + (Math.random() - 0.5) * 0.4)),
        ]),
      ),
    }));
    ...
    setClassStats({
      ...
      at_risk: demoStudents.filter((s) => s.risk_level === 'high' || s.risk_level === 'critical')
        .length,
    ...
    <div className="container max-w-6xl py-8 space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card className={classStats.at_risk > 0 ? 'border-red-500/30' : ''}>
          <CardContent className="pt-4 text-center">
            <AlertTriangle
              className={`h-6 w-6 mx-auto mb-1 ${classStats.at_risk > 0 ? 'text-red-500' : 'text-emerald-500'}`}
            />
            <div
              className={`text-3xl font-extrabold ${classStats.at_risk > 0 ? 'text-red-500' : 'text-emerald-500'}`}
            >
              {classStats.at_risk}
            </div>
        ...
        <Card>
          <CardContent className="pt-4 text-center">
            ...
            <div className="text-3xl font-extrabold">
              {(classStats.avg_mastery * 100).toFixed(0)}%
            </div>
      ...
      <Tabs defaultValue="attrition" className="space-y-6">
        <TabsList className="grid w-full grid-cols-3 lg:w-[500px]">
          <TabsTrigger value="attrition" className="gap-1">
            <AlertTriangle className="h-4 w-4" /> Attrition Radar
          </TabsTrigger>
          <TabsTrigger value="heatmap" className="gap-1">
            <BarChart3 className="h-4 w-4" /> Skill Heatmap
          </TabsTrigger>
          <TabsTrigger value="interventions" className="gap-1">
            <Lightbulb className="h-4 w-4" /> Interventions
          </TabsTrigger>
        </TabsList>
              ...
              <Card key={student.id} className={`${config.border} transition-colors`}>
                <CardContent className="py-4">
                  <div className="flex items-center gap-4">
                    {/* Risk indicator */}
                    <div
                      className={`h-12 w-12 rounded-full ${config.bg} flex items-center justify-center shrink-0`}
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge
                          className={`${config.bg} ${config.color} border-0 text-[10px] uppercase`}
                        >
                          {student.risk_level}
                        </Badge>
                      </div>
                    ...
                    <div className="hidden md:flex items-center gap-4 text-xs text-muted-foreground">
                      ...
                      <span className="text-center">
                        <div className="font-bold text-foreground">
                          {(student.mastery_avg * 100).toFixed(0)}%
                        </div>
                        Mastery
                      </span>
                    </div>
        ...
        <TabsContent value="heatmap">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Mastery scores across skills for each student. 🔴 &lt;30% | 🟡 30-65% | 🟢 &gt;65%
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr>
                      <th className="text-left p-2 font-semibold text-muted-foreground sticky left-0 bg-background">
                        Student
                      </th>
                      {skillNames.map((skill) => (
                        <th
                          key={skill}
                          className="p-2 font-semibold text-muted-foreground text-center min-w-16"
                        >
                          {skill}
                        </th>
                      ))}
        ...
        <TabsContent value="interventions" className="space-y-4">
          <Card className="bg-amber-500/5 border-amber-500/20">
            <CardHeader>
              <CardTitle className="text-base flex items-center gap-2">
                <Lightbulb className="h-5 w-5 text-amber-500" /> AI-Powered Intervention
                Recommendations
              </CardTitle>
              <CardDescription>
                Based on the Dropout Risk Predictor model, here are suggested actions for at-risk
                students.
              </CardDescription>
            </CardHeader>
          </Card>
          ...
          {students.filter((s) => s.risk_level === 'critical' || s.risk_level === 'high').length ===
            0 && (
