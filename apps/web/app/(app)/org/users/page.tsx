...
import { Eye, Pencil, ShieldAlert, ShieldCheck, Trash2, User } from 'lucide-react';
  ...
  const columns: Column<any>[] = [
    {
      ...
      render: (user) => (
        <>
          <div
            className={`font-semibold transition-colors ${!user.is_active ? 'opacity-70' : 'group-hover:text-primary text-foreground'}`}
          >
            {user.full_name}
          </div>
          <div
            className={`text-xs mt-1 ${!user.is_active ? 'text-muted-foreground line-through opacity-70' : 'text-muted-foreground'}`}
          >
            {user.email}
          </div>
        </>
    ...
    },
    {
      ...
      render: (user) => (
        <Badge
          variant="outline"
          className="bg-blue-500/10 text-blue-600 border-blue-500/20 capitalize shadow-sm"
        >
          {user.role.replace('_', ' ')}
        </Badge>
      ),
    ...
    <>
      ...
      <ConfirmDialog
        ...
        description={
          <>
            ...
            <span className="font-bold">{userToDelete?.name}</span> from this organization? This
      ...
      <Tabs defaultValue="all" className="w-full mt-6">
        <TabsList className="mb-6 bg-muted/50">
          <TabsTrigger
            value="all"
            className="text-sm font-medium px-6 rounded-md data-[state=active]:bg-background data-[state=active]:shadow-sm"
          >
            All Users{' '}
