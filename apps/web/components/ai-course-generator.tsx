...
import { 
  Loader2, Sparkles, BookOpen, CheckCircle2, Video, 
  MapPin, Clock, Save, Layers, ArrowUpRight, GraduationCap, Globe, Check, Code2, Edit3, RefreshCw, Play
} from "lucide-react";
...
function renderLectureContent(material: string) {
  ...
  lines.forEach((line, i) => {
    ...
    if (trimmed.startsWith("```")) {
      if (inCodeBlock) {
        elements.push(
          <div key={`code-${i}`} className="my-5 rounded-2xl bg-slate-950 p-5 font-mono text-sm text-emerald-400 border border-slate-800 shadow-xl overflow-x-auto">
    ...
    if (trimmed.startsWith("### ")) {
      elements.push(
        <h3 key={i} className="text-xl font-black text-primary mt-6 mb-3 border-b border-primary/20 pb-2">
          {trimmed.replace("### ", "")}
        </h3>
      );
      ...
      elements.push(
        <div key={i} className="flex items-start gap-3 text-sm text-foreground/90 my-2 pl-3">
          ...
          <span className="leading-relaxed">{renderFormattedText(trimmed.replace(/^[-*]\s+/, ""))}</span>
        </div>
      );
    } else if (/^\d+\.\s+/.test(trimmed)) {
      elements.push(
        <div key={i} className="flex items-start gap-3 text-sm text-foreground/90 my-2 pl-3 font-medium">
          <span className="bg-primary/15 text-primary font-black text-xs px-2 py-0.5 rounded-md shrink-0 shadow-xs">
            {trimmed.match(/^\d+\./)?.[0]}
          </span>
          <span className="leading-relaxed">{renderFormattedText(trimmed.replace(/^\d+\.\s+/, ""))}</span>
        </div>
      );
    } else {
      elements.push(
...
export function AiCourseGenerator() {
  ...
  const getDynamicLessonContent = (topicName: string, lessonTitle: string) => {
    ...
    if (lower.includes("architecture") || lower.includes("virtual dom") || lower.includes("foundations")) {
...
- Keep components small, focused, and pure whenever possible.`;
      ...
      videos = [
        { id: "SqcY0GlETPk", title: "React Architecture & Virtual DOM in 100 Seconds", channel: "Fireship" },
        { id: "8JafUJg5jFk", title: "How React Virtual DOM Works Under the Hood", channel: "Web Dev Simplified" },
        { id: "bMknfKXIFA8", title: "React Architecture - Full Masterclass", channel: "freeCodeCamp" },
        { id: "Tn6-PIqc4UM", title: "React Component Tree & Reconciliation", channel: "Programming with Mosh" }
      ];
    } 
    else if (lower.includes("setup") || lower.includes("environment") || lower.includes("tooling")) {
...
- Inspect component re-renders using React DevTools Profiler.`;
      ...
      videos = [
        ...
        { id: "Tn6-PIqc4UM", title: "Setting up Node.js & React Environment", channel: "Programming with Mosh" },
        { id: "erEgovGjbEs", title: "React Developer Tools Deep Dive", channel: "Web Dev Simplified" },
        { id: "hdI2bqOjy3c", title: "npm & Node environment configuration", channel: "Traversy Media" }
      ];
    } 
    else if (lower.includes("syntax") || lower.includes("component") || lower.includes("data") || lower.includes("props")) {
...
- Use destructuring for cleaner prop signatures.`;
      ...
      videos = [
        { id: "w7ejDZ8SWv8", title: "React Components & Props Walkthrough", channel: "Traversy Media" },
        { id: "bMknfKXIFA8", title: "JSX Syntax & Component Architecture", channel: "freeCodeCamp" },
        { id: "Tn6-PIqc4UM", title: "Functional Components vs Class Components", channel: "Programming with Mosh" },
        { id: "SqcY0GlETPk", title: "JSX Expressions in 100 Seconds", channel: "Fireship" }
      ];
    } 
    else if (lower.includes("state") || lower.includes("hook") || lower.includes("application") || lower.includes("working")) {
...
- Clean up subscriptions or timers in the useEffect return function.`;
      ...
      videos = [
        { id: "TNhaISOUy6Q", title: "React Hooks Course - useState & useEffect", channel: "Web Dev Simplified" },
        { id: "bMknfKXIFA8", title: "Building Your First State-Driven App", channel: "freeCodeCamp" },
        { id: "SqcY0GlETPk", title: "React Hooks Overview in 100 Seconds", channel: "Fireship" },
        { id: "w7ejDZ8SWv8", title: "React State Management & Practical App", channel: "Traversy Media" }
      ];
    } 
    else if (lower.includes("routing") || lower.includes("navigation") || lower.includes("api") || lower.includes("server")) {
...
- Centralize API base URLs in environment configuration.`;
      ...
      videos = [
        { id: "59IXY5IDubY", title: "React Router v6 Complete Tutorial", channel: "Web Dev Simplified" },
        { id: "bMknfKXIFA8", title: "Connecting React Frontend to REST API", channel: "freeCodeCamp" },
        { id: "w7ejDZ8SWv8", title: "Axios API Requests & React Integration", channel: "Traversy Media" },
...
- Continuously refactor complex logic into reusable helper functions.`;
      ...
      videos = [
        { id: "bMknfKXIFA8", title: `${cleanTitle} — Full Engineering Guide`, channel: "freeCodeCamp" },
        { id: "SqcY0GlETPk", title: `${cleanTopic} Core Principles in 100 Seconds`, channel: "Fireship" },
        { id: "w7ejDZ8SWv8", title: `${cleanTitle} Hands-on Tutorial`, channel: "Traversy Media" },
        { id: "Tn6-PIqc4UM", title: `${cleanTopic} Best Practices Masterclass`, channel: "Programming with Mosh" }
      ];
    }
  ...
  const handleSaveCourse = async () => {
    ...
    try {
      ...
      toast.success("Course & Interactive Lessons saved to Database!");
    } catch (err) {
  ...
  const openLessonModal = (moduleTitle: string, lessonInput: any) => {
    ...
    const lessonObj = {
      ...
      duration: typeof lessonInput === "object" && lessonInput.duration ? lessonInput.duration : "15 mins",
      summary: typeof lessonInput === "object" && lessonInput.summary ? lessonInput.summary : `Detailed workspace for ${titleStr}`,
      lecture_material: (typeof lessonInput === "object" && lessonInput.lecture_material && lessonInput.lecture_material.length > 100)
        ? lessonInput.lecture_material
        : dynamicData.lectureMaterial,
      gfg_url: (typeof lessonInput === "object" && lessonInput.gfg_url && !lessonInput.gfg_url.includes("/search/"))
        ? lessonInput.gfg_url
        : dynamicData.gfgUrl,
      docs_url: typeof lessonInput === "object" && lessonInput.docs_url ? lessonInput.docs_url : dynamicData.docsUrl,
      ...
      {showForm && (
        <Card className="max-w-2xl border-primary/20 shadow-md">
          <CardHeader>
            ...
            <CardDescription>Enter topic and target audience to build an interactive course roadmap.</CardDescription>
          </CardHeader>
          ...
          <CardFooter>
            <Button onClick={handleGenerate} disabled={loading} className="w-full gap-2 font-semibold">
              {loading ? (
                <>
                  <Loader2 className="h-4 w-4 animate-spin" /> Generating Interactive Course & Lessons...
                </>
              ) : (
                <>
      ...
      {result && (
        <div className="space-y-6 mt-4">
          ...
          <Card className="border-primary/20 bg-gradient-to-br from-primary/10 via-background to-primary/5 shadow-sm">
            <CardHeader className="pb-4">
              <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                <div className="flex items-center gap-2">
                  <Badge variant="outline" className="uppercase text-[10px] font-bold tracking-wider border-primary/30 text-primary">
                ...
                <Button onClick={handleSaveCourse} disabled={saving} size="sm" className="gap-2 font-semibold">
              ...
              <CardDescription className="text-sm text-muted-foreground">
                {result.roadmap?.description || `Tailored learning trajectory designed for ${targetAudience}.`}
              </CardDescription>
              ...
              <div className="flex flex-wrap items-center gap-4 pt-4 text-xs font-medium border-t border-primary/10 mt-3">
                ...
                <div className="flex items-center gap-1.5 text-muted-foreground">
                  ...
                  <span>Click any lesson below to read lecture notes, watch videos & GeeksforGeeks</span>
                </div>
                <div className="flex items-center gap-1.5 text-muted-foreground">
          ...
          <Card className="border-border/80">
            <CardHeader className="border-b bg-muted/20 pb-4">
              <CardTitle className="flex items-center gap-2 text-lg">
                <MapPin className="h-5 w-5 text-primary" /> 1. Visual Learning Roadmap & Interactive Lessons
              </CardTitle>
              <CardDescription>Click on any lesson pill to open its dedicated lecture content, YouTube video, and GeeksforGeeks tutorial</CardDescription>
            </CardHeader>
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-center gap-3 min-w-0 pr-2">
                                  <span className={`flex h-6.5 w-6.5 items-center justify-center rounded-full text-[11px] font-bold shrink-0 ${
                                    isDone 
                                ...
                                <div className="flex items-center gap-1 shrink-0">
                                  <Badge variant="outline" className="text-[10px] font-bold text-primary border-primary/20 bg-primary/5 gap-1">
          ...
          <Card>
            <CardHeader className="border-b bg-muted/20 pb-4">
              <CardTitle className="flex items-center gap-2 text-lg">
                <CheckCircle2 className="h-5 w-5 text-primary" /> 2. Practice Quizzes & Diagnostic Checks
              </CardTitle>
              <CardDescription>Multiple choice questions to evaluate learner comprehension</CardDescription>
            </CardHeader>
            <CardContent className="pt-6">
          ...
          <DialogContent className="sm:max-w-4xl md:max-w-5xl lg:max-w-6xl w-[94vw] max-h-[92vh] overflow-y-auto p-6 sm:p-8 shadow-2xl border-primary/20 rounded-2xl bg-card">
            <DialogHeader className="space-y-3 border-b pb-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  <Badge variant="outline" className="text-xs uppercase font-extrabold tracking-wide text-primary border-primary/40 px-3 py-1 bg-primary/5">
                    {selectedLesson.moduleTitle || "Course Lesson"}
                  </Badge>
                  <Badge variant="secondary" className="text-xs px-2.5 py-1 font-semibold">
                    <Clock className="h-3.5 w-3.5 inline mr-1 text-muted-foreground" /> {selectedLesson.duration || "15 mins"}
                  </Badge>
                </div>
              ...
              <TabsContent value="video" className="space-y-6 pt-6">
                <div className="flex items-center justify-between border-b pb-3">
                  <div>
                    <h4 className="font-extrabold text-lg text-foreground flex items-center gap-2">
                      <Play className="h-5 w-5 text-red-500 fill-current" /> Curated Video Lectures for {selectedLesson.title}
                    </h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {selectedLesson.videos?.map((vid: any, vIdx: number) => (
                    <div key={vIdx} className="rounded-2xl border bg-card overflow-hidden shadow-sm hover:shadow-md transition-all flex flex-col justify-between space-y-3">
                      ...
                      <div className="p-4 pt-1 space-y-2">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <span className="text-[10px] text-muted-foreground font-semibold">HD 1080p</span>
                        </div>
                        <h5 className="font-bold text-sm text-foreground line-clamp-1">{vid.title}</h5>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  <div className="p-6 rounded-2xl border bg-card hover:bg-emerald-500/5 transition-all space-y-4 border-emerald-500/30 shadow-xs flex flex-col justify-between">
                    <div className="space-y-3">
                      <div className="flex items-center gap-3">
                        ...
                        <h4 className="font-extrabold text-base text-foreground">GeeksforGeeks Article</h4>
                      </div>
                      <p className="text-sm text-muted-foreground leading-relaxed">
                        Access detailed tutorial articles, code snippets, syntax reference, and interview prep guides directly on GeeksforGeeks.
                      </p>
                    </div>
                    <div className="pt-4">
                      <a 
                  ...
                  <div className="p-6 rounded-2xl border bg-card hover:bg-blue-500/5 transition-all space-y-4 border-blue-500/30 shadow-xs flex flex-col justify-between">
                    <div className="space-y-3">
                      <div className="flex items-center gap-3">
                        ...
                        <h4 className="font-extrabold text-base text-foreground">Official Documentation</h4>
                      </div>
                      <p className="text-sm text-muted-foreground leading-relaxed">
                        Refer to official developer documentation, MDN Web Docs, and API reference specifications.
                      </p>
                    </div>
                    <div className="pt-4">
                      <a 
